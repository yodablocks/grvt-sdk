...
Walks through the full order pipeline:
  1. Authenticate with an API key
    ...
    Everything targets TESTNET by default.  Set ENV=mainnet to go live.
    """
...
CHAIN_ID            = 326
VERIFYING_CONTRACT  = "0x0000000000000000000000000000000000000000"  # fill in real address
...
def rest_demo() -> None:
    ...
    # 4. Build an order (limit buy 0.001 BTC @ $50,000)
...
async def ws_demo() -> None:
    ...
    async def on_trade(msg: dict) -> None:
              ...
              logger.info("[trade]  %s  price=%s  size=%s",
                          INSTRUMENT,
                          data.get("price", "?"),
                          data.get("size",  "?"))
    ...
    async def on_book(msg: dict) -> None:
          ...
          async def on_order_update(msg: dict) -> None:
                    data = msg.get("data", msg)
                    logger.info("[order]  id=%s  status=%s",
                                data.get("order_id", "?"),
                                data.get("status",   "?"))
...
def main() -> None:
      # REST portion (synchronous)
      rest_demo()

    # WebSocket portion (async)
      asyncio.run(ws_demo())
